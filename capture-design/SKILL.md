---
name: capture-design
description: Capture a design language from reference assets (screenshots, images, live URLs, product photos, video stills) into a reusable, named design-system document plus saved references — a "swipefile" entry that future projects can import to recreate the aesthetic. Use this whenever the user shares visual material and says anything like "capture this design", "add this to my swipefile", "extract this design system", "I love this aesthetic, save it", "design inspo", or drops screenshots/URLs with the intent of reusing the look later. Also use when the user wants to document the visual identity of a product, site, poster, piece of hardware, or brand for later application.
---
...
If Python is available, sampling dominant colors from images beats eyeballing:

```python
# pip install Pillow --break-system-packages
from PIL import Image
img = Image.open("ref.png").convert("RGB")
colors = sorted(img.getcolors(maxcolors=10**6), reverse=True)[:20]
print([('#%02x%02x%02x' % c, n) for n, c in colors])
```
...
1. **Save the references.** Copy every provided asset into `references/` at the project root, naming files descriptively (`landing-hero.png`, `detail-typography.png`). Do this first, and verify the files actually exist on disk before moving on — this step fails silently in some environments:
   - Images pasted into chat are often visible to you but never materialized as files (the uploads folder is empty). Check the uploads folder; if the assets aren't there, tell the user immediately and ask them to re-attach as files or connect a folder, then continue analysis in parallel rather than blocking.
